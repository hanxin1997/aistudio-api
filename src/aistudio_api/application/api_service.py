...
from aistudio_api.infrastructure.settings.global_settings_store import GlobalSettings, get_global_settings_store
...
def _build_thinking_overrides(thinking: str | None) -> dict | None:
    if thinking is None:
        return None
    key = thinking.strip().lower()
    if key == "off":
        return {"thinking_config": None}
    level = _THINKING_LEVEL_MAP.get(key)
    if level is None:
        return None
    return {"thinking_config": AistudioThinkingConfig(level=level).to_wire()}


def _resolve_effective_thinking(req_thinking: str | None, global_settings: GlobalSettings) -> str | None:
    if req_thinking is not None:
        return req_thinking
    default = (global_settings.default_thinking or "off").lower()
    if default in {"off", "low", "medium", "high"}:
        return default
    return None


def _apply_search_mode_to_tools(
    tools: list[list] | None,
    model: str,
    global_settings: GlobalSettings,
) -> list[list] | None:
    mode = global_settings.google_search_mode
    google_search_template = TOOLS_TEMPLATES["google_search"]
    has_search = bool(tools) and any(t == google_search_template for t in tools)

    if mode == "always_off":
        if not tools:
            return None
        filtered = [t for t in tools if t != google_search_template]
        return filtered or None

    if mode == "always_on":
        if has_search:
            return tools
        return [*(tools or []), google_search_template]

    if tools is None and any(m in model for m in ("gemma-4-26b-a4b-it", "gemma-4-31b-it")):
        return [google_search_template]
    return tools


def _apply_global_settings_to_normalized(normalized: dict, global_settings: GlobalSettings) -> bool:
    """把全局设置应用到 normalize_gemini_request 输出。返回 safety_off。"""
    normalized["tools"] = _apply_search_mode_to_tools(
        normalized.get("tools"), normalized["model"], global_settings
    )

    overrides = dict(normalized.get("generation_config_overrides") or {})
    if "thinking_config" not in overrides:
        thinking_overrides = _build_thinking_overrides(global_settings.default_thinking)
        if thinking_overrides is not None:
            overrides.update(thinking_overrides)
    normalized["generation_config_overrides"] = overrides or None

    return global_settings.safety == "off"
    ...
    for attempt in range(max_retries):
        async with busy_lock:
            ...
            try:
                ...
                global_settings = get_global_settings_store().get()
                tools = normalize_openai_tools(req.tools)
                tools = _apply_search_mode_to_tools(tools, model, global_settings)

                effective_thinking = _resolve_effective_thinking(req.thinking, global_settings)
                generation_config_overrides = _build_thinking_overrides(effective_thinking)
                safety_off = global_settings.safety == "off"
    ...
    for attempt in range(max_retries):
        async with busy_lock:
            ...
            try:
                ...
                global_settings = get_global_settings_store().get()
                safety_off = _apply_global_settings_to_normalized(normalized, global_settings)
                logger.info(
                ...
                if stream:
                    return _build_gemini_streaming_response(
                        client=client, normalized=normalized, safety_off=safety_off
                    )
...
def _build_gemini_streaming_response(*, client: AIStudioClient, normalized: dict, safety_off: bool = False) -> StreamingResponse:
    async def stream_response():
        busy_lock = runtime_state.busy_lock
        if busy_lock is None:
