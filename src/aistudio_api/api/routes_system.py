...
from aistudio_api.infrastructure.settings.global_settings_store import get_global_settings_store
...
async def get_global_settings():
    settings = get_global_settings_store().get()
    return GlobalSettingsResponse(**settings.to_dict())


@router.put("/settings/global", response_model=GlobalSettingsResponse)
async def update_global_settings(req: GlobalSettingsUpdateRequest):
    try:
        settings = get_global_settings_store().update(
            google_search_mode=req.google_search_mode,
            safety=req.safety,
            default_thinking=req.default_thinking,
        )
