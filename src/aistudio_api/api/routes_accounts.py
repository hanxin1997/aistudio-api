...
async def login_status(
    ...
    return LoginStatusResponse(
        ...
        email=session.email,
        error=session.error,
        vnc_ws_port=session.vnc_ws_port,
        vnc_path=session.vnc_path,
    )
...
async def export_account(
    ...
    auth_path = account_service._store.get_auth_path(account_id)
    if auth_path is None:
        raise HTTPException(status_code=404, detail="auth.json 不存在")
    safe = re.sub(r"[^A-Za-z0-9._-]", "_", account.email or account.name or account_id)
    return FileResponse(
        path=str(auth_path),
        media_type="application/json",
        filename=f"{safe}-{account_id}.json",
    )
...
@router.post("/import-auth", response_model=ImportAuthResponse)
async def import_auth(
    req: ImportAuthRequest,
    account_service=Depends(get_account_service),
):
    """从 Playwright storage_state JSON 导入账号（对应导出按钮的反向操作）。"""
    state = req.storage_state
    if not isinstance(state, dict) or "cookies" not in state:
        raise HTTPException(status_code=400, detail="无效的 storage_state（缺少 cookies 字段）")

    cookies = state.get("cookies") or []
    if not cookies:
        raise HTTPException(status_code=400, detail="cookies 为空")
    ...
    log.info("[import-auth] saved account %s with %d cookies", account.id, len(cookies))
